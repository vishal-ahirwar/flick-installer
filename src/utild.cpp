...
// /
//
static QVector<QPair<QUrl, QString>> tools{
...
Utild::Utild(QObject* parent) : QObject{parent}, _current_index{0}
{
    // Connect signals with queued connection to ensure proper sequence
    connect(this, &Utild::downloadingFinished, this, &Utild::onDowloadingFinished,
            Qt::QueuedConnection);
    connect(this, &Utild::installingFinished, this, &Utild::addToPath, Qt::QueuedConnection);
    connect(this, &Utild::addToPathFinished, this, &Utild::onAddToPathFinished,
            Qt::QueuedConnection);
...
QString Utild::fileName() const { return _file_name; }
...
void Utild::download(const QUrl& url, const QString& file_path)
{
    ...
    QTimer::singleShot(
        ...
        [this, file_path, url]()
        {
            ...
            connect(res, &QNetworkReply::errorOccurred, this,
                    [this, file_path, res]()
                    {
                        emit downloadFailed("Error while downloading " + file_path + ": " +
                                            res->errorString());
                    });

            connect(
                res, &QNetworkReply::downloadProgress, this,
                [this](auto rec, auto total)
                {
                    if (total > 0)
                    {
            ...
            connect(res, &QNetworkReply::finished, this,
                    [res, this, file_path]()
                    {
                        ...
                        {
                            setFilName("Download failed for: " + file_path +
                                       " with error: " + res->errorString());
                        }
...
void Utild::installing()
{
    ...
    QTimer::singleShot(
        ...
        [this]()
        {
            ...
            connect(worker, &InstallerWorker::finished, this,
                    [this, thread, worker]()
                    {
                        thread->quit();
                        thread->wait();
...
void Utild::onDowloadingFinished() { installing(); };
void Utild::onInstallingFinished()
{
    bool isInstallingVCPKG{false};
    QProcess process;
    process.start("vcpkg", QStringList() << "--version");
    if (!process.waitForFinished() || process.error() == QProcess::FailedToStart)
    {
        mIsInstallingVCPKG=true;
    };
    addToPath();
};
...
bool Utild::can_procceed() const { return m_can_procceed; }
